...
extern "C" {
    JNIEXPORT void JNICALL Java_com_example_android_camera2video_CodecUtils_copyFlexYUVImage(JNIEnv *env,
        ...
        jclass /*clazz*/, jobject target, jobject source)
{
    ...
    if (tgt != NULL && src != NULL) {
        __android_log_print(ANDROID_LOG_INFO, TAG, "copyFlexYUVImage %dx%d (%d,%d..%d,%d) (%zux%zu) %+zd%+zd %+zd%+zd %+zd%+zd <= "
